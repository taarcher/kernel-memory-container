...
int memory_container_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    if( tempO != NULL ){
        ...
        mutex_unlock( &mtx );
        printk( KERN_DEBUG "return address for object %d", tempO->oid);
        return ( tempO->memory );
    }
...
//    printk( KERN_DEBUG "start: %d", vma->vm_start );
    printk( KERN_DEBUG "allocate memory" );
    addr = kmalloc( size, GFP_KERNEL );
    newObj->memory = addr;
    printk( KERN_DEBUG "copy the address to obj node" );
//    copy_from_user( &newObj->memory, vma->vm_start, sizeof (unsigned long) );
    printk( KERN_DEBUG "remap" );
//    int ret = remap_pfn_range( vma, vma->vm_start, virt_to_phys(kmalloc( size, GFP_KERNEL ) ) >> PAGE_SHIFT, size, vma->vm_page_prot );
    int ret = remap_pfn_range( vma, vma->vm_start, virt_to_phys( addr ) >> PAGE_SHIFT, size, vma->vm_page_prot );

    //throw away to make it run?
    
    if (ret != 0 ){
        printk(KERN_DEBUG "remap failed" );
    }
    //copy_from_user( &addr, vma->vm_start, sizeof (unsigned long));
//    newObj->memory = addr;
    printk( KERN_DEBUG "vma address is %d", newObj->memory);
...
//        newCon->prev = head;
        ...
        struct ObjectNode* obj = kmalloc( sizeof (struct ObjectNode), GFP_KERNEL );
        newCon->start_o = obj;
