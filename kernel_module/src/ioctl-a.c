...
int memory_container_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    int check = 0;
    check = remap_pfn_range( vma, vma->vm_start, virt_to_phys(kmalloc( size, GFP_KERNEL ) ) >> PAGE_SHIFT, size, vma->vm_page_prot );
    printk( KERN_DEBUG "remap_pfn_range result: %d", check );
